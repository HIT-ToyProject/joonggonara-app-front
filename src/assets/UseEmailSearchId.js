...
const UseEmailSearchId = () => {
  ...
  const requestVerificationCode = async () => {
    if (!emailInput || !nameInput || emailErrorText) {
      setErrorStatus(true);
      return;
    }

    const url = "http://localhost:9090/user/login/findId/email";
    const findUserIdByEmailRequest = {
      name: nameInput,
      email: emailInput,
    };
    try {
  ...
  const checkVerificationCode = async () => {
    ...
    const url =
      "http://localhost:9090/user/login/checkVerificationCode/userId?verificationType=EMAIL";
...
export default UseEmailSearchId;
